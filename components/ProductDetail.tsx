  ...
  return (
    <>
      ...
      <div className="detailInfo">
        <p className="detailKicker" style={{ "--i": 0 } as React.CSSProperties}>{product.category}{product.badge ? ` · ${product.badge}` : ""}</p>
        ...
        <div className="includesPanel" style={{ "--i": 4 } as React.CSSProperties}>
          <h3>Lo que trae <small>{product.items.length} productos</small></h3>
          <ul>
            {product.items.map((item, i) => {
              const visual = imageFor(item);
              return (
                <li key={item} style={{ "--j": i } as React.CSSProperties}>
        ...
        <div className="buyBox" style={{ "--i": 6 } as React.CSSProperties}>
          ...
          <button type="button" className="btnV2 btnV2Solid addBtn" data-added={added || undefined} onClick={addToCart}>
          ...
          <p className="addFollow" data-show={added || undefined}>
            <Link href="/carrito">Ver carrito <span aria-hidden="true">→</span></Link>
          </p>
        ...
        <ul className="perks" style={{ "--i": 7 } as React.CSSProperties}>
          <li><strong>Delivery programado</strong><small>Eliges día y horario en Lima</small></li>
          <li><strong>Boleta o factura</strong><small>Comprobante electrónico</small></li>
          <li><strong>Lista para regalar</strong><small>Con lazo y tarjeta</small></li>
        </ul>
      </div>
    </>
  );
