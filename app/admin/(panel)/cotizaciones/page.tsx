...
import { fechaCorta, numero } from "@/lib/admin/format";
  ...
  return (
    <>
      <header className="admHead">
        <div>
          <h1>Cotizaciones</h1>
          <p className="admMuted">Llegan desde el formulario de empresas o las registras tú (pedidos por WhatsApp o correo). Al aprobarlas se convierten en órdenes de pedido.</p>
        </div>
      </header>

      <nav className="admTabs" aria-label="Filtrar por estado">
        <Link href="/admin/cotizaciones/nueva" className="admTabAgregar">+ AGREGAR</Link>
      ...
      <div className="admCard admCardFlush">
        {cotizaciones.length === 0 ? (
          ...
          <table className="admTable">
            <thead><tr><th>N.º cotización</th><th>Fecha de creación</th><th>Nombre del cliente</th><th>Contacto</th><th className="num">Cantidad</th><th>Presupuesto</th><th>Estado</th></tr></thead>
            <tbody>
              {cotizaciones.map((c) => (
                <tr key={c.id}>
