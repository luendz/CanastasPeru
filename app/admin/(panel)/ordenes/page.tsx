  ...
  return (
    <>
      ...
      <form className="admFilters" method="get">
        ...
        <select className="admInput" name="canal" defaultValue={canal}>
          <option value="">Todos los orígenes</option>
          {CANALES.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
        </select>
        <button className="admBtn" type="submit">Filtrar</button>
        {(estado || canal || q) && <Link className="admLinkMuted" href="/admin/ordenes">Limpiar</Link>}
      </form>
      ...
      <div className="admCard admCardFlush">
        {ordenes.length === 0 ? (
          ...
          <table className="admTable">
            <thead>
              <tr><th>N.º</th><th>Creada</th><th>Cliente</th><th>Origen</th><th>Entrega</th><th className="num">Total</th><th>Pago</th><th>Estado</th><th><span className="admSrOnly">PDF</span></th></tr>
            </thead>
            <tbody>
              {ordenes.map((o) => (
                <tr key={o.id}>
                  <td><Link className="admStrongLink" href={`/admin/ordenes/${o.id}`}>{o.numero}</Link></td>
                  <td>{fechaCorta(o.created_at)}</td>
                  <td>{o.cliente_nombre}<small className="admMuted admBlock">{o.cliente_email ?? o.cliente_telefono ?? ""}</small></td>
                  <td>{labelCanal(o.canal)}{o.origen === "cotizacion" && <small className="admMuted admBlock">Cotización</small>}</td>
                  ...
                  <td><EstadoRapido id={o.id} numero={o.numero} estado={o.estado} /></td>
                  <td><BotonPdfOrden id={o.id} numero={o.numero} compacto /></td>
                </tr>
              ))}
            </tbody>
          </table>
